...
class Music(commands.Cog):
    ...
    async def join(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None) -> bool:
        if not channel:
        ...
        if node is not None:
            await ctx.author.voice.channel.connect(cls=node.get_player(ctx.guild.id))
        else:
            await ctx.send("There was an error getting the node")
            return False
    ...
    @command(aliases=['disconnect', 'dc', 'disc', 'lv', 'fuckoff'])
